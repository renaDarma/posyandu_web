<?php

namespace App\Controllers;

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
use Dompdf\Dompdf;
use Dompdf\Options;
use App\Models\OrtuModel;
use Exception;

class Ortu extends BaseController
{
    protected $ortuModel;
    protected $db;
    protected $user;
    public function __construct()
    {
        $this->ortuModel = new OrtuModel();
        $this->db = \Config\Database::connect();
        $this->user = $this->db->table('users');
        date_default_timezone_set('Asia/Jakarta');
    }

    public function index()
    {
        $currentPage = $this->request->getVar('page_ortu') ? $this->request->getVar('page_ortu') : 1;

        $ortu = new OrtuModel();
      
        $data  = [
            'pager' => $this->ortuModel->pager,
            'currentPage' => $currentPage
        ];
        $data['title'] = 'Data Orang Tua Anak';
        $data['ortu'] = $ortu->getAllData(); 

        return view('kader/ortu/dataortu', $data);
    }


    public function tambahortu()
    {
        $data  = [
            'title' => 'Tambah Data Orang Tua',
            'validation' => \Config\Services::validation()
        ];

        return view('kader/ortu/tambahortu', $data);
    }

    public function save()
    {
        $data_user = [
            'username' => $this->request->getVar('ibunik'),
            'email' => $this->request->getVar('ibunik') . '@mail.com',
            'password_hash' => password_hash($this->request->getVar('ibunik'), PASSWORD_BCRYPT),
            'fullname' => $this->request->getVar('ibunama'),
            'usernohp' => str_replace("-","", $this->request->getVar('ibunohp')),
            'active' => '1',
            'role'  => 'masyarakat',
            'created_at' => date("Y-m-d H:i:s"),
            'updated_at' => date("Y-m-d H:i:s"),
        ];
        $this->user->insert($data_user);

        $user_id = $this->user->orderBy('id', 'desc')->get()->getRow();
        $data = [
            'id_user' => $user_id->id,
            'nokk' => $this->request->getVar('nokk'),
            'ayahnik' => $this->request->getVar('ayahnik'),
            'ayahnama' => $this->request->getVar('ayahnama'),
            'ayahtmptlhr' => $this->request->getVar('ayahtmptlhr'),
            'ayahtgllhr' => $this->request->getVar('ayahtgllhr'),
            'ayahagama' => $this->request->getVar('ayahagama'),
            'ayahpendidikan' => $this->request->getVar('ayahpendidikan'),
            'ayahpekerjaan' => $this->request->getVar('ayahpekerjaan'),
            'ayahnohp' => $this->request->getVar('ayahnohp'),
            'ibunik' => $this->request->getVar('ibunik'),
            'ibunama' => $this->request->getVar('ibunama'),
            'ibutmptlhr' => $this->request->getVar('ibutmptlhr'),
            'ibutgllhr' => $this->request->getVar('ibutgllhr'),
            'ibuagama' => $this->request->getVar('ibuagama'),
            'ibupendidikan' => $this->request->getVar('ibupendidikan'),
            'ibupekerjaan' => $this->request->getVar('ibupekerjaan'),
            'ibunohp' => $this->request->getVar('ibunohp'),
            'alamat' => $this->request->getVar('alamat'),
            'jmlhanak' => $this->request->getVar('jmlhanak')
        ];

        if(!$this->validate([
                'nokk' => [
                    'rules' => 'required|is_unique[ortu.nokk]
                    |is_unique[ortu.ayahnik]|is_unique[ortu.ibunik]
                    |is_unique[anak.nik]',
                    'errors' => [
                        'required' => '{field} wajib diisi.',
                        'is_unique' => '{field} sudah terdaftar.'
                    ]
                ],
                'ayahnik' => [
                    'rules' => 'required|is_unique[ortu.ayahnik]|is_unique[ortu.nokk]
                    |is_unique[ortu.ibunik]|is_unique[anak.nik]',
                    'errors' => [
                        'required' => '{field} wajib diisi.',
                        'is_unique' => '{field} sudah terdaftar.'
                    ]
                ],
                'ayahnama' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ayahtmptlhr' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ayahtgllhr' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ayahagama' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ayahpendidikan' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ayahpekerjaan' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ayahnohp' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ibunik' => [
                    'rules' => 'required|is_unique[ortu.ibunik]
                    |is_unique[ortu.ayahnik]|is_unique[ortu.nokk]
                    |is_unique[anak.nik]',
                    'errors' => [
                        'required' => '{field} wajib diisi.',
                        'is_unique' => '{field} sudah terdaftar.'
                    ]
                ],
                'ibunama' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ibutmptlhr' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ibutgllhr' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ibuagama' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ibupendidikan' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ibupekerjaan' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'ibunohp' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ],
                'alamat' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ], 
                'jmlhanak' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => '{field} wajib diisi.'
                    ]
                ]
            ]))
            if (!$this->ortuModel->validate($data)) {
                session()->setFlashdata('error','Mohon cek kembali data Anda!');
                return redirect()->to('/tambahortu')->withInput()->with('errors', $this->ortuModel->errors());
            }
            
            try {
                $this->ortuModel->protect(false)->insert($data);
            } catch (Exception $e) {
                return redirect()->to('/tambahortu')->withInput()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
            }

            session()->setFlashdata('pesan', 'Data berhasil ditambahkan.');
            return redirect()->to('/dataortu');
    }

    public function detailortu($idortu)
    {
        $data  = [
            'title' => 'Detail Data Orang Tua Anak',
            'ortu' => $this->ortuModel->getOrtu($idortu),
            'validation' => \Config\Services::validation()
        ];

        return view('kader/ortu/detailortu', $data);
    }

    public function editortu($idortu)
    {
        $data  = [
            'title' => 'Edit Data Orang Tua Anak',
            'ortu' => $this->ortuModel->getOrtu($idortu),
            'validation' => \Config\Services::validation()
        ];

        return view('kader/ortu/editortu', $data);
    }

    public function update($idortu)
    {
        //cek nokk
        $nokkLama = $this->ortuModel->getOrtu($this->request->getVar('idortu'));
        if($nokkLama['nokk'] == $this->request->getVar('nokk')){
            $rule_nokk = 'required';
        }else{
            $rule_nokk = 'required|is_unique[ortu.nokk] 
            |is_unique[ortu.ayahnik]|is_unique[ortu.ibunik]
            |is_unique[anak.nik]';
        }

         //cek nokk
         $ayahnikLama = $this->ortuModel->getOrtu($this->request->getVar('idortu'));
         if($ayahnikLama['ayahnik'] == $this->request->getVar('ayahnik')){
             $rule_ayahnik = 'required';
         }else{
            $rule_ayahnik = 'required|is_unique[ortu.nokk] 
            |is_unique[ortu.ayahnik]|is_unique[ortu.ibunik]
            |is_unique[anak.nik]';
         }

          //cek nokk
        $ibunikLama = $this->ortuModel->getOrtu($this->request->getVar('idortu'));
        if($ibunikLama['ibunik'] == $this->request->getVar('ibunik')){
            $rule_ibunik = 'required';
        }else{
            $rule_ibunik = 'required|is_unique[ortu.nokk]
            |is_unique[ortu.ayahnik]|is_unique[ortu.ibunik]
            |is_unique[anak.nik]';
        }

        $idortu = $this->request->getVar('idortu');
        if(!$this->validate([
            'nokk' => [
                'rules' => $rule_nokk,
                'errors' => [
                    'required' => '{field} wajib diisi.',
                    'is_unique' => '{field} sudah terdaftar.'
                ]
            ],
            'ayahnik' => [
                'rules' => $rule_ayahnik,
                'errors' => [
                    'required' => '{field} wajib diisi.',
                    'is_unique' => '{field} sudah terdaftar.'
                ]
            ],
            'ayahnama' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ayahtmptlhr' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ayahtgllhr' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ayahagama' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ayahpendidikan' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ayahpekerjaan' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ayahnohp' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ibunik' => [
                'rules' => $rule_ibunik,
                'errors' => [
                    'required' => '{field} wajib diisi.',
                    'is_unique' => '{field} sudah terdaftar.'
                ]
            ],
            'ibunama' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ibutmptlhr' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ibutgllhr' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ibuagama' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ibupendidikan' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ibupekerjaan' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'ibunohp' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ],
            'alamat' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ], 
            'jmlhanak' => [
                'rules' => 'required',
                'errors' => [
                    'required' => '{field} wajib diisi.'
                ]
            ]  
            ])
            ){
                return redirect()->to('/editortu/' . $idortu)->withInput();
            }
    
            $this->ortuModel->save([
                'idortu' => $idortu,
                'nokk' => $this->request->getVar('nokk'),
                'ayahnik' => $this->request->getVar('ayahnik'),
                'ayahnama' => $this->request->getVar('ayahnama'),
                'ayahtmptlhr' => $this->request->getVar('ayahtmptlhr'),
                'ayahtgllhr' => $this->request->getVar('ayahtgllhr'),
                'ayahagama' => $this->request->getVar('ayahagama'),
                'ayahpendidikan' => $this->request->getVar('ayahpendidikan'),
                'ayahpekerjaan' => $this->request->getVar('ayahpekerjaan'),
                'ayahnohp' => $this->request->getVar('ayahnohp'),
                'ibunik' => $this->request->getVar('ibunik'),
                'ibunama' => $this->request->getVar('ibunama'),
                'ibutmptlhr' => $this->request->getVar('ibutmptlhr'),
                'ibutgllhr' => $this->request->getVar('ibutgllhr'),
                'ibuagama' => $this->request->getVar('ibuagama'),
                'ibupendidikan' => $this->request->getVar('ibupendidikan'),
                'ibupekerjaan' => $this->request->getVar('ibupekerjaan'),
                'ibunohp' => $this->request->getVar('ibunohp'),
                'alamat' => $this->request->getVar('alamat'),
                'jmlhanak' => $this->request->getVar('jmlhanak')
            ]);
         session()->setFlashdata('pesan', 'Data berhasil diedit.');
 
         return redirect()->to('/dataortu');
    }

    public function hapusortu($idortu)
    {
        $this->ortuModel->delete($idortu);
        session()->setFlashdata('pesan', 'Data berhasil dihapus.');
        return redirect()->to('/dataortu');
    }

    public function pdf()
    {
        $data  = [
            'ortu' => $this->ortuModel->getAllData(),
            'title' => 'PDF Daftar Data Orang Tua Anak',
        ];

        // $filename = 'Daftar Data Orang Tua ('. date('d-m-y').')';
        $filename = 'Daftar Data Orang Tua Anak';

        // instantiate and use the dompdf class
        $dompdf = new Dompdf();

        // Mengatur opsi Dompdf
        $options = new Options();
        $options->set('isRemoteEnabled', true);

        // Inisialisasi Dompdf dengan opsi yang telah ditentukan
        $dompdf = new Dompdf($options);

        // load HTML content
        $dompdf->loadHtml(view('kader/ortu/pdfortu', $data));

        // (optional) setup the paper size and orientation
        $dompdf->setPaper('A4', 'landscape');

        // render html as PDF
        $dompdf->render();

        // output the generated pdf
        $dompdf->stream($filename, ['Attachment' => false]);
    }

    // public function viewexcel()
    // {
    //     return view('kader/ortu/dataortu');
    // }

    public function excel()
    {
        $data  = [
            'ortu' => $this->ortuModel->getAllData(),
            'title' => 'Excel Daftar Data Orang Tua Anak',
        ];

        // Membuat objek Spreadsheet
        $spreadsheet = new Spreadsheet();

        // Mendapatkan lembar kerja aktif dan Menambahkan data ke sel 
        $spreadsheet->setActiveSheetIndex(0)
            ->setCellValue('A1', 'No')
            ->setCellValue('B1', 'No KK')
            ->setCellValue('C1', 'NIK Ayah')
            ->setCellValue('D1', 'Nama Ayah')
            ->setCellValue('E1', 'Tempat Lahir Ayah')
            ->setCellValue('F1', 'Tanggal Lahir Ayah')
            ->setCellValue('G1', 'Agama Ayah')
            ->setCellValue('H1', 'Pendidikan Terakhir Ayah')
            ->setCellValue('I1', 'Pekerjaan Ayah')
            ->setCellValue('J1', 'No HP Ayah')
            ->setCellValue('K1', 'NIK Ibu')
            ->setCellValue('L1', 'Nama Ibu')
            ->setCellValue('M1', 'Tempat Lahir Ibu')
            ->setCellValue('N1', 'Tanggal Lahir Ibu')
            ->setCellValue('O1', 'Agama Ibu')
            ->setCellValue('P1', 'Pendidikan Terakhir Ibu')
            ->setCellValue('Q1', 'Pekerjaan Ibu')
            ->setCellValue('R1', 'No HP Ibu')
            ->setCellValue('S1', 'Alamat')
            ->setCellValue('T1', 'Jmlh Anak');

        $no = 1;
        $column = 2;

        // Mengatur lebar kolom A secara otomatis
        foreach ($data['ortu'] as $nak) {
            $spreadsheet->setActiveSheetIndex(0)
                ->setCellValue('A' . $column, $no++)
                ->setCellValue('B' . $column, $nak->nokk)
                ->setCellValue('C' . $column, $nak->ayahnik)
                ->setCellValue('D' . $column, $nak->ayahnama)
                ->setCellValue('E' . $column, $nak->ayahtmptlhr)
                ->setCellValue('F' . $column, $nak->ayahtgllhr)
                ->setCellValue('G' . $column, $nak->ayahagama)
                ->setCellValue('H' . $column, $nak->ayahpendidikan)
                ->setCellValue('I' . $column, $nak->ayahpekerjaan)
                ->setCellValue('J' . $column, $nak->ayahnohp)
                ->setCellValue('K' . $column, $nak->ibunik)
                ->setCellValue('L' . $column, $nak->ibunama)
                ->setCellValue('M' . $column, $nak->ibutmptlhr)
                ->setCellValue('N' . $column, $nak->ibutgllhr)
                ->setCellValue('O' . $column, $nak->ibuagama)
                ->setCellValue('P' . $column, $nak->ibupendidikan)
                ->setCellValue('Q' . $column, $nak->ibupekerjaan)
                ->setCellValue('R' . $column, $nak->ibunohp)
                ->setCellValue('S' . $column, $nak->alamat)
                ->setCellValue('T' . $column, $nak->jmlhanak);
        
            $column++;
        }

        $writer = new Xlsx($spreadsheet);
        // $filename = 'Daftar Data Orang Tua Anak ('. date('y-m-d-H-i-s').')';
        $filename = 'Daftar Data Orang Tua Anak';

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename=' . $filename . '.xlsx');
        header('Cache-Control: max-age=0');

        $writer->save('php://output');
    }
    

}